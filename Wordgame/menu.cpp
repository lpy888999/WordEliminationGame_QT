...
Creator::Creator(string username) :Player(username){
	...
	//请求发送玩家Creator信息
	tcp.sendMsg(7, username);
	string c_info = tcp.recvMsg();
	istringstream ss(c_info); //创建一个istringstream对象，以str作为输入源
	string seginfo; //临时字符串
	ss >> seginfo;
	_nickname = seginfo;
	ss >> seginfo;
	_level = atoi(seginfo.c_str());
	ss >> seginfo;
	_exp = atoi(seginfo.c_str());
	std::cout << "my exp:" << _exp << std::endl;//测试
	ss >> seginfo;
	_wordnum = atoi(seginfo.c_str());
